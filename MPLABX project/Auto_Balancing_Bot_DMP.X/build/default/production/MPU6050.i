    ...
    do
    {
        if ((fifoC = MPU6050_getFIFOCount()) > length)
        {
            if (fifoC > 200) {
                MPU6050_resetFIFO();
                fifoC = 0;
                while (!(fifoC = MPU6050_getFIFOCount()) && ((time_ms - BreakTimer) <= 11));
            }
                ...
                while ((fifoC = MPU6050_getFIFOCount()) > length) {
                    ...
                    while (fifoC) {
                        RemoveBytes = (int)fifoC < 32 ? fifoC:32;
        ...
        if ((time_ms - BreakTimer) > 11) return 0;
...
_Bool MPU6050_writeMemoryBlock(const uint8_t *data, uint16_t dataSize, uint8_t bank, uint8_t address, _Bool verify, _Bool useProgMem) {
    ...
    uint16_t i;
    uint8_t j;
    if (verify) verifyBuffer = mlc1;
    if (useProgMem) progBuffer = mlc2;
    for (i = 0; i < dataSize;) {
        ...
        I2Cdev_writeBytes(mpu6050.devAddr, 0x6F, chunkSize, progBuffer);


        if (verify && verifyBuffer) {
            MPU6050_setMemoryBank(bank, 0, 0);
            MPU6050_setMemoryStartAddress(address);
            I2Cdev_readBytes(mpu6050.devAddr, 0x6F, chunkSize, verifyBuffer);
...
_Bool MPU6050_writeDMPConfigurationSet(const uint8_t *data, uint16_t dataSize, _Bool useProgMem) {
    uint8_t mlc[16], success, special;
    uint8_t* progBuffer;
    uint16_t i, j;
    if (useProgMem)
        progBuffer = (uint8_t *)mlc;
    ...
    for (i = 0; i < dataSize;) {
        ...
        if (length > 0) {
            ...
            success = MPU6050_writeProgMemoryBlock(progBuffer, length, bank, offset, 1);
            i += length;
        } else {
            ...
            if (special == 0x01) {
                ...
                I2Cdev_writeByte(mpu6050.devAddr, 0x38, 0x32);
...
uint8_t SaveAddress = (ReadAddress == 0x3B)?((MPU6050_getDeviceID() < 0x38 )? 0x06:0x77):0x13;
 ...
 for (int i = 0; i < 3; i++) {
  I2Cdev_readWords(mpu6050.devAddr, SaveAddress + (i * shift), 1, (uint16_t *)&Data);
  Reading = Data;
  if(SaveAddress != 0x13){
   BitZero[i] = Data & 1;
 ...
 for (int L = 0; L < Loops; L++) {
  ...
  for (int c = 0; c < 100; c++) {
   eSum = 0;
   for (int i = 0; i < 3; i++) {
    I2Cdev_readWords(mpu6050.devAddr, ReadAddress + (i * 2), 1, (uint16_t *)&Data);
    Reading = Data;
    if ((ReadAddress == 0x3B)&&(i == 2)) Reading -= 16384;
    Error = -Reading;
    eSum += abs(Reading);
    PTerm = kP * Error;
    ITerm[i] += (Error * 0.001) * kI;
    if(SaveAddress != 0x13){
                    Data = roundf((PTerm + ITerm[i] ) / 8);
     Data = ((Data)&0xFFFE) |BitZero[i];
    } else Data = roundf((PTerm + ITerm[i] ) / 4);
    I2Cdev_writeWords(mpu6050.devAddr, SaveAddress + (i * shift), 1, (uint16_t *)&Data);
   }
   if((c == 99) && eSum > 1000){
    c = 0;

   }
   if((eSum * ((ReadAddress == 0x3B)?.05: 1)) < 5) eSample++;
   if((eSum < 100) && (c > 10) && (eSample >= 10)) break;
   _delay((unsigned long)((1)*(_X_FREQ/4000.0)));
  }
  ...
  for (int i = 0; i < 3; i++){
   if(SaveAddress != 0x13) {
    Data = roundf((ITerm[i] ) / 8);
    Data = ((Data)&0xFFFE) |BitZero[i];
   } else Data = roundf((ITerm[i]) / 4);
   I2Cdev_writeWords(mpu6050.devAddr, SaveAddress + (i * shift), 1, (uint16_t *)&Data);
  }
 }
 MPU6050_resetFIFO();
